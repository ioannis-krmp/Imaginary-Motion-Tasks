...
# def calculate_dfa_channel(signal_channel):
#   return dfa(signal_channel)

# def calculate_dfa(root_path):
#   num_processes = 8 # set the number of processes to use
#   fileExists = True
#   dfa_list = []
#   for pilot in range(1, 34):
#     events_channels_array = np.zeros((63, 20))
...
#           dfa_values = p.map(calculate_dfa_channel, [signal[channel] for channel in range(num_channels)])
#           for channel in range(num_channels):
#             events_channels_array[channel][event-1] = dfa_values[channel]
#       else:
#         fileExists = False
#     if(fileExists): 
#       for channel in range(num_channels):
#         mean_same_channel = np.mean(events_channels_array[channel][:])
#         dfa_list.append(mean_same_channel)
#   print(np.array(dfa_list).shape)
#   return np.array(dfa_list)
